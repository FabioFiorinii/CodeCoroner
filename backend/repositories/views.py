...
class RepositoryViewSet(viewsets.ModelViewSet):
    ...
    def index(self, request, pk=None):  # noqa: ARG002
    ...
    def reindex(self, request, pk=None):  # noqa: ARG002
        repo = self.get_object()
    ...
    def status_detail(self, request, pk=None):  # noqa: ARG002
        repo = self.get_object()
        return Response(
            {
                'id': str(repo.id),
    ...
    def files(self, request, pk=None):  # noqa: ARG002
        repo = self.get_object()
    ...
    @action(detail=True, methods=['get'])
    def chunks(self, request, pk=None):  # noqa: ARG002
        repo = self.get_object()
        chunks = (
            CodeChunk.objects.filter(file__repository=repo)
            .select_related('file')
            .order_by('file__file_path', 'start_line')
        )
